...
FINGERPRINT_PATH = _find_realistic_data()
...
def load_fingerprints():
    ...
    try:
        ...
        print("[FUSION] " + "!"*60)
        print(f"[FUSION] WARNING: fingerprint file NOT FOUND")
        print(f"[FUSION]   expected at: {FINGERPRINT_PATH}")
        print(f"[FUSION] Consequences - the system will NOT crash, it will")
        print(f"[FUSION] silently degrade:")
        print(f"[FUSION]   - localization falls back to the variance heuristic")
        print(f"[FUSION]     (which penalises corner seats - the v5 defect)")
        print(f"[FUSION]   - multi-seat disambiguation is DISABLED")
        print(f"[FUSION]     (one device may produce several alerts)")
        print(f"[FUSION] Fix: commit rssi_realistic_summary.json to")
        print(f"[FUSION]   <repo>/RF/simulation/  or regenerate it with")
        print(f"[FUSION]   rf_realistic_dataset_generator.py")
        print("[FUSION] " + "!"*60)
    return _fingerprints
